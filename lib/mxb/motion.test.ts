...
test("throttle / in-game wheelie leans the deck back, brake pitches it forward", () => {
  const gas = run(sample({ speedMs: 12, throttle: 1, accelG: { x: 0, y: 1, z: 0.8 }, pitch: -14 }), 0.8);
  const brake = run(sample({ speedMs: 12, frontBrake: 1, accelG: { x: 0, y: 1.1, z: -1 }, pitch: 12 }), 0.8);
  assert.ok(gas.pitch > 0.12, `gas pitch ${gas.pitch}`);
  assert.ok(brake.pitch < -0.12, `brake pitch ${brake.pitch}`);
  assert.ok(visualPitch(gas.pitch) < 0, `visual wheelie ${visualPitch(gas.pitch)}`);
});
...
test("compressed shocks lift the deck on a 1 G whoop", () => {
  ...
  assert.ok(pose.y > 0.02, `y ${pose.y}`);
});
...
test("live wheelie (negative m_fPitch) leans the garage back and ignores surge G", () => {
  const expected = (22 * Math.PI) / 180;
  const withG = run(
    sample({ speedMs: 16, throttle: 1, pitch: -22, accelG: { x: 0, y: 1, z: 0.7 } }),
    0.75,
  );
  const noG = run(
    sample({ speedMs: 16, throttle: 1, pitch: -22, accelG: { x: 0, y: 1, z: 0 } }),
    0.75,
  );
  assert.ok(withG.pitch > 0.25, `lean back ${withG.pitch}`);
  assert.ok(Math.abs(withG.pitch - expected) < 0.08, `follow ${withG.pitch} vs ${expected}`);
  assert.ok(visualPitch(withG.pitch) < -0.25, `visual ${visualPitch(withG.pitch)}`);
  assert.ok(Math.abs(withG.pitch - noG.pitch) < 0.03, `G nod ${withG.pitch} vs ${noG.pitch}`);
});
...
test("Three.js pitch is inverted so a PiBoSo wheelie draws nose-up", () => {
  ...
  const gas = run(sample({ speedMs: 12, throttle: 1, accelG: { x: 0, y: 1, z: 0.2 }, pitch: -14 }), 0.8);
